"""
AI Agents Bot - V4.1 Stocks Fixed - حل مشكلة f-string
"""
import os
import re
...
app = FastAPI(title="AI Agents Bot - V4.1 Stocks Fixed", version="4.1")
...
إذا سأل عن سهم: إذا أعطيتك بيانات السعر، حللها مع تنبيه أنها ليست نصيحة استثمارية.
تحدث عربية بلهجة سعودية خفيفة، ردودك مختصرة للواتساب.
"""
...
def home():
    ...
    <div class="card">
        <h2>📈 البوت شغال - V4.1 أسهم لحظية (Fixed)</h2>
        <div><span class="badge badge-green">__STATUS__</span><span class="badge">WhatsApp: __WA_STATUS__</span><span class="badge badge-blue">Stocks: Live ✅</span><span class="badge badge-orange">__MODEL__</span></div>
    ...
    <div class="card" style="font-size:12px">
        <b>Endpoints:</b> <code>/stock/2222</code> , <code>/stock/1120</code> , <code>/stock/AAPL</code> , <code>/stocks?symbols=2222,1120,2010</code> , <code>/health</code>
    </div>
...
الوقت: ${data.time}`;
    ...
    """
    html = html.replace("__STATUS__", status).replace("__WA_STATUS__", wa_status).replace("__MODEL__", model_name)
    return HTMLResponse(content=html)

@app.get("/health")
def health():
    return {"status": "ok", "version": "4.1-stocks-fixed", "groq": bool(client), "model": model_name, "stocks": "live"}
...
def chat_endpoint(req: ChatRequest):
    ...
    if detected:
        ...
        if stock_data:
            ...
            stock_context += "\nاستخدم هذه البيانات في تحليلك. تنبيه: ليست نصيحة استثمارية.\n"
    try:
        messages = [
        ...
        return {"reply": res.choices[0].message.content, "stocks_detected": detected}
    except Exception as e:
